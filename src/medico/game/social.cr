...
module Social
  ...
  def fame_change(doc, patient, outcome, random = DEF_RND) : Int32
    if outcome
      ...
      result = stat_to_int(rate * doc.skill_power(Medico::Advertising, 20, random), random)
      result = random.rand(result) unless patient.is_good
    else
      rate = patient.status.level * patient.starting_health
      result = -stat_to_int(rate / doc.skill_power(Medico::Advertising, 50, random), random)
    end
    result
  end
